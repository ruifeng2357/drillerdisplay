...
public class RivCrossDDService extends Fragment { //01/14 subclass Fragment
    ...
    /**
     * This thread runs while listening for incoming BT connections. It behaves
    ...
    /**
     * This thread runs during a BT connection with a remote device.
    ...
    private class ConnectedThread extends Thread {
        ...
        final byte delim = 5; //02/14 there are no packets in BT, so use an unprintable character as a "packet" delimiter
        boolean keepGoing = true; //02/14 used to prevent auto-restarting the connection if it was purposely closed in order to switch to Wifi mode
        ...
        public void run() {
            ...
            byte[] packetBytes = new byte[128]; //02/14 temporarily holds all data received in the last call to mmInStream.read()
            byte[] readBuffer = new byte[128]; //02/14 temporarily holds a subset of packetBytes that belongs to a single "packet"
            ...
            while (keepGoing) {
                try {
                	...
                	if(mmInStream.available() == 0) { //02/14 wait up to 3s for data to be received
                    ...
                    for(int i=0;i<bytes;i++) {
                        if(packetBytes[i] == delim) { //send only one "packet" at a time to avoid parsing errors and subsequent data loss
        ...
        public void cancel() {
        	keepGoing = false; //02/14 indicate that this thread should not be auto-restarted
        	if(mmInStream != null) {
    ...
    public class WIFIclientThread extends Thread implements Runnable {
		...
		public void run() {
			...
			try {
				...
				if(D) Log.d("Dial", "Server: Start connecting");
				socket = new DatagramSocket(serverPort, serverAddr);
				socket.setSoTimeout(3000); //02/14 wait up to 3s for data to be received
				byte[] buf = new byte[128]; //02/14 make a lot bigger so that it can hold the entire contents of a message
				...
				while (keepGoing) {
					try {
						...
						if(keepGoing && !noRCDataToastShown) {
							setState(STATE_LISTEN); //02/14 change connection status to not connected if no data is being received
							Message msg = mHandler.obtainMessage(MainActivity.MESSAGE_TOAST);
					        Bundle bundle = new Bundle();
					...
					//02/14 send correct number of bytes to UI handler
				...
				if(D) Log.e(TAG, "disconnected", e);
				if(e.getMessage().contains(ERR_BAD_IP)) { //02/14 handle bad IP addresses gracefully (show a message, don't try to reconnect)
					setState(STATE_LISTEN);
					Message msg = mHandler.obtainMessage(MainActivity.MESSAGE_TOAST);
			        Bundle bundle = new Bundle();
			        ...
			        msg.setData(bundle);
			        mHandler.sendMessage(msg);
				} else if(e.getMessage().contains(ERR_BAD_PORT)) { //02/14 handle bad port #s gracefully (show a message, don't try to reconnect)
					setState(STATE_LISTEN);
					Message msg = mHandler.obtainMessage(MainActivity.MESSAGE_TOAST);
			        Bundle bundle = new Bundle();
			        ...
			        msg.setData(bundle);
			        mHandler.sendMessage(msg);
				} else if(keepGoing) connectionLost(); //02/14 otherwise, attempt to reconnect (if the wifi connection wasn't canceled)
			}
		}
		
		//02/14 used to indicate that the wifi thread should stop and not be restarted
		public void cancel() {
			keepGoing = false;
            try {
            	if(socket != null) socket.close();
            } catch (Exception e) {
            	if(D) Log.e(TAG, "Wifi Socket close() of server failed", e);
            }
		}
	}
}
