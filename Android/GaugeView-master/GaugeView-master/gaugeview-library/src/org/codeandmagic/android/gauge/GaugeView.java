...
public class GaugeView extends View {
	...
	public static final float OUTER_SHADOW_WIDTH = 0.02f;
	...
	public static final float INNER_RIM_BORDER_WIDTH = 0.003f;
	...
	public static final float SCALE_END_VALUE = 360.0f;
	public static final float SCALE_START_ANGLE = 0.0f;
	public static final int SCALE_DIVISIONS = 9;
	public static final int SCALE_SUBDIVISIONS = 8;
	...
	public static final float[] RANGE_VALUES = { 0.1f, 0.2f, 360.0f };
	public static final int[] RANGE_COLORS = { Color.rgb(255, 255, 255), Color.rgb(255, 255, 255), Color.rgb(255, 255, 255) };
	//02/14 added separate range colors for pressure gauges (to show red warning zones)
	public static final int[] RANGE_COLORS_GAUGES = { Color.rgb(255, 0, 0), Color.rgb(255, 255, 255), Color.rgb(255, 0, 0) };
	...
	public static final float TEXT_DIVISION_TICK_SIZE = 0.06f;
	...
	private float mTextDivisionTickSize;
	...
	private float mDivisionAngle; //02/14 added
	...
	public GaugeView(final Context context, final AttributeSet attrs, final int defStyle) {
		...
		//01/14 This was causing problems in the MainActivity designer. It sort of works now, rather than not working at all.
		//if(!isInEditMode()){
	...
	private void readAttrs(final Context context, final AttributeSet attrs, final int defStyle) {
		...
		if (mShowText) {
			...
			mTextDivisionTickSize = a.getFloat(R.styleable.GaugeView_textDivisionTickSize, TEXT_DIVISION_TICK_SIZE);
		}
	...
	private void readRanges(final Resources res, final int rangesId, final int colorsId) {
		if (rangesId > 0 && colorsId > 0) {
			...
			//02/14 all white for compass-style, red-white-red for gauge-style
			if(mScaleStartAngle == 0) mRangeColors = RANGE_COLORS;
			else mRangeColors = RANGE_COLORS_GAUGES;
		}
	}
	...
	//02/14 given low and high limits (from app settings), determine size of additional red warning sections and create an appropriate # of divisions
	//(only used with pressure gauges)
	public void setRanges(float lowLimit, float highLimit) {
		float redSection = (float) (0.15 * (highLimit - lowLimit));
		mScaleStartValue = lowLimit - redSection;
		mScaleEndValue = highLimit + redSection;
		mDivisionValue = getTicIncrements();
		mSubdivisions = (mDivisionValue % 8 == 0 ? 8 : 5);
		initScale();
		mRangeValues = new float[] { lowLimit + mSubdivisionValue, highLimit, mScaleEndValue };
		invalidate();
	}
	
	//02/14 breaks pressure range into a reasonable round # of divisions. Logic taken from RivCross pressure dials
	private float getTicIncrements() {
		double range = mScaleEndValue - mScaleStartValue;
		double value = 0.01;
		while(value < 10000) {
			if(range / value < 12) return (float)(value);
			if(range / (value*2) < 12) return (float)(value*2);
			if(range / (value*5) < 12) return (float)(value*5);
			value *= 10;
		}
		return 10000;
	}
	...
	public void setDefaultScaleRangePaints() {
		...
		for (int i = 0; i < length; i++) {
			...
			mRangePaints[i].setStyle(Paint.Style.FILL); //02/14 looks better than STROKE
			mRangePaints[i].setTextSize(1f); //02/14 set to 1 since any smaller will cause rendering issues
	...
	public Paint getDefaultTextValuePaint() {
		...
		paint.setStyle(Paint.Style.FILL); //02/14 looks better than FILL_AND_STROKE
		paint.setTextSize(1); //02/14 set to 1 since any smaller will cause rendering issues
	...
	private void initScale() {
		//02/14 keep track of division angle in addition to subdivision angle
		if(mScaleStartAngle == 0) {
			mScaleRotation = 0; //02/14 start drawing tick marks at the top for compass-style gauges
			mDivisionValue = (mScaleEndValue - mScaleStartValue) / mDivisions;
			mDivisionAngle = (360 - 2 * mScaleStartAngle) / mDivisions;
		} else { //not a compass-style gauge. mDivisionValue should already have been set in setRanges
			mScaleRotation = (mScaleStartAngle + 180) % 360;
			mDivisionAngle = (360 - 2 * mScaleStartAngle) * (mDivisionValue / (mScaleEndValue - mScaleStartValue));
		}
		mSubdivisionValue = mDivisionValue / mSubdivisions;
		mSubdivisionAngle = mDivisionAngle / mSubdivisions;
	}
...
// 02/14 draw the gauge in onDraw instead, so that invalidate() ends up redrawing it
//	@Override
...
//	}
	...
	private void drawGauge() {
		...
		if((getWidth() == 0) || (getHeight() == 0)) return; //01/14 exit if height or width is 0 (since this function will be called again)
	...
	protected void onDraw(final Canvas canvas) {
		drawGauge(); //02/14 do first (onSizeChanged occurs before onDraw)
	...
	private void drawText(final Canvas canvas) {
		final String textValue = !TextUtils.isEmpty(mTextValue) ? mTextValue : valueString(mCurrentValue, 1);
		...
		//02/14 scale the canvas to account for the fact that the font size was increased to prevent rendering problems
		canvas.save();
		canvas.scale(mTextValueSize, mTextValueSize);
		canvas.drawText(textValue, startX / mTextValueSize, startY / mTextValueSize, mTextValuePaint);
		canvas.restore();
	...
	private void drawScale(final Canvas canvas) {
		...
		// CW no rotate for AHSx - I want 0 on top!!!  
		canvas.rotate(mScaleRotation, 0.5f, 0.5f);

		//02/14 determining how many tick marks are needed at the start is more complicated now, so make this a "while(not done)"-style loop
		float valuesTraversed = 0;
		float diff = ((int)(mScaleEndValue / mSubdivisionValue) * mSubdivisionValue - (int)(mScaleStartValue / mSubdivisionValue) * mSubdivisionValue);
		if(mScaleStartAngle == 0) diff -= mSubdivisionValue; //02/14 for compass-style gauges, we don't need the last tick mark because it's the same as the first tick mark
		int i = 0;
		while(valuesTraversed <= diff) {
			...
			if (value % mDivisionValue == 0) {
				// Draw a division tick
				canvas.drawLine(0.5f, y1, 0.5f, y3, paint);
				// Draw the text 0.15 away from the division tick
				//02/14 scale the canvas to account for the fact that the font size was increased to prevent rendering problems
				canvas.save();
				canvas.scale(mTextDivisionTickSize, mTextDivisionTickSize);
				canvas.drawText(valueString(value, 0), 0.5f / mTextDivisionTickSize, (y3 + 0.045f) / mTextDivisionTickSize, paint);
				canvas.restore();
			}
			else {
			...
			valuesTraversed += mSubdivisionValue;
	...
	//02/14 add numDecPlaces in order to show decimal places for gauge value but not for gauge division tick marks
	private String valueString(final float value, int numDecPlaces) {
		if(numDecPlaces < 1) return String.format(Locale.US, "%d", (int)value);
		else return String.format(Locale.US, "%." + numDecPlaces + "f", value);
	}

	//02/14 start at the first possible value on the gauge, rounding towards the nearest zero-ward tick mark
	private float getValueForTick(final int tick) {
		return (((int)(mScaleStartValue / mSubdivisionValue)) * mSubdivisionValue) + (tick * (mDivisionValue / mSubdivisions));
	}
	...
	private float getAngleForValue(final float value) {
		return (mScaleRotation + ((value - (int)(mScaleStartValue / mSubdivisionValue) * mSubdivisionValue) / mSubdivisionValue) * mSubdivisionAngle) % 360;
	}

	// TODO needle velocity and/or acceleration is jerky. Make it smooth again.
	private void computeCurrentValue() {
		...
		if (-1 != mNeedleLastMoved) {
			...
			if(mScaleStartAngle==0) { //add modulo stuff for compass-style gauges so that the needle can cross the boundary between low and high limit
			...
			if(mScaleStartAngle==0) {
				while(mCurrentValue < 0) mCurrentValue += 360;
				while(mCurrentValue >= 360) mCurrentValue -= 360;
			}
	...
	public void setTargetValue(final float value) {
		...
		//02/14 always make the gauge show the actual pressure reading, even if it's beyond the end of a red warning zone
		mTextValue = valueString(value, 1);
