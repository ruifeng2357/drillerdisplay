...
public class DDPrefs extends Activity {
	...
	public static class DDPrefsFrag extends PreferenceFragment {//03/14 changed to public static to prevent crashing when screen is rotated while in settings activity
	    ...
	    public void onCreate(Bundle savedInstanceState) {
	        ...
	        if(!PreferenceManager.getDefaultSharedPreferences(getActivity()).getBoolean(MainActivity.PREF_BT_MODE, true)) { //03/14 make sure IP and port settings start out enabled if in Wifi mode
	        	...
	        	if(this.getArguments().getBoolean(MainActivity.DISABLE_BT_MODE)) btMode.setEnabled(false); //prevent user from switching to BT mode if BT isn't supported
	        }
