...
#define TEXTSIZE(size, text, attr, font) \
...
- (void)initialize;
{
    ...
    background = nil;
    bgImage = [UIImage imageNamed:@"gauge"];
...
- (void)initDrawingRects
{
    ...
    innerRimRect = CGRectMake(0.05, 0.05, 0.95, 0.95);;
...
- (void)drawText:(CGContextRef)context
{
    ...
    CGSize fontWidth;
    if ([_unitOfMeasurement respondsToSelector:@selector(sizeWithAttributes:)])
...
- (void)drawScale:(CGContextRef)context
{
    ...
    for (int i = 0; i < totalTicks; i++)
    {
        ...
        if ((abs(mod) < 1 /*0.000001*/) || (abs(mod) < 1 /*0.000001*/))
        {
            ...
            // Draw label
            BOOL drawThisValue = YES;
            if (value == self.maxValue && self.scaleStartAngle == 0 && self.scaleEndAngle == 360)
                drawThisValue = NO;
            if (drawThisValue == YES)
            {
                ...
                CGSize fontWidth;
                if ([valueString respondsToSelector:@selector(sizeWithAttributes:)])
                ...
                else
                {
                    ...
                    fontWidth = [attributedText boundingRectWithSize:(CGSize){1, 1} options:NSStringDrawingUsesLineFragmentOrigin context:nil].size;
                }
...
- (void) drawStringAtContext:(CGContextRef) context string:(NSString*)text withCenter:(CGPoint)center_ radius:(CGFloat)radius startAngle:(CGFloat)startAngle endAngle:(CGFloat)endAngle
{
    ...
    for (int index = 0; index < [text length]; index++)
    {
        ...
        CGSize charSize;
        TEXTSIZE(charSize, text, stringAttrs, font);
        //= [letter sizeWithAttributes:stringAttrs];
  
        float totalWidth;
        CGSize totalSize;
        //= [[NSString stringWithFormat:@"%@%@", lastLetter, letter] sizeWithAttributes:stringAttrs].width;
        NSString *str = [NSString stringWithFormat:@"%@%@", lastLetter, letter];
        TEXTSIZE(totalSize, str, stringAttrs, font)
        totalWidth = totalSize.width;
        CGSize currentLetterSize;
        TEXTSIZE(currentLetterSize, letter, stringAttrs, font);
        float currentLetterWidth = currentLetterSize.width;
        CGSize lastLetterSize;
        TEXTSIZE(lastLetterSize, lastLetter, stringAttrs, font);
        float lastLetterWidth = lastLetterSize.width;
